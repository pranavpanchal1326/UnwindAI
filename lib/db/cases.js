...
export async function getFullCaseState(caseId) {
  // GAP-10: Instant DEMO_MODE bypass
  if (process.env.DEMO_MODE === 'true') {
    try {
      const demoData = await import('@/DEMO_RESPONSES/dashboard_meera.json')
      return {
        case:          demoData.default.case,
        profile:       demoData.default.profile,
        professionals: demoData.default.professionals,
        decisions:     demoData.default.decisions_pending,
        activeTasks:   demoData.default.tasks || []
      }
    } catch (err) {
      console.warn('[Demo] Could not load dashboard_meera.json, using skeleton')
      return null
    }
  }
