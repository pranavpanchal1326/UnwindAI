...
/**
 * withFallback
 * High-level compatibility wrapper for structured.js
 * @param {string} key - Provider key (e.g., 'emotion', 'orchestrator')
 * @param {object} options - Call options (system, prompt, messages, etc.)
 */
export async function withFallback(key, options) {
  const { groqProvider, geminiProvider } = await import('./providers')

  // Map key to providers
  ...
  return callWithFallback(providers, prompt, {
    ...options,
    systemPrompt
  })
}