...
// Memory cache for demo responses
const _demoCache = {}
...
export async function getDemoResponse(fileName) {
  if (typeof window !== 'undefined') {
    throw new Error('getDemoResponse can only be called on the server')
  }

  if (_demoCache[fileName]) {
    return _demoCache[fileName]
  }

  try {
    const fs = await import('fs')
    const path = await import('path')
    const filePath = path.join(process.cwd(), 'DEMO_RESPONSES', `${fileName}.json`)
    const rawData = fs.readFileSync(filePath, 'utf8')
    const data = JSON.parse(rawData)
    _demoCache[fileName] = data
    return data
  } catch (err) {
    throw new Error(
