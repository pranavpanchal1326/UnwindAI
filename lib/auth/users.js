...
import { createSupabaseServerClient } from '../db/server.js'
...
export async function getCurrentUser() {
  ...
  if (process.env.DEMO_MODE === 'true') {
    return {
      authUser: { id: 'demo_user_auth_id', email: 'meera@example.com' },
      dbUser: {
        id: 'demo_user_db_id',
        email: 'meera@example.com',
        case_id: 'CASE_MEERA_DEMO_001',
        consent_emotion_shield: true,
        private_mode_enabled: false,
  ...
  if (!dbUser) {
    // Allow fallback even if authUser exists but DB record missing
    if (process.env.DEMO_MODE === 'true') {
      return {
        authUser,
        dbUser: {
